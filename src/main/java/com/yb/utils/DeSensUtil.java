...
public class DeSensUtil {
    ...
    public static void desensitize(PageUtils page, Map<String, String> deSens) {   //对分页对象中的列表数据进行脱敏
        if (null != page && !CollectionUtils.isEmpty(page.getList())) {
            desensitize(page.getList(), deSens);
        }
    }

    public static void desensitize(List<?> objects, Map<String, String> deSens) {  // 对集合中每个元素脱敏
    ...
    public static void desensitize(Object obj, Map<String, String> deSens) {  //对单个对象及其所有字段（包括父类字段）脱敏
