...
import { getInfoAPI, saveAPI, updateAPI } from '@/api/list'
import { toDetailPage, toListPage } from '@/utils/navigate'
import { crossCalFn, csuEvent, getCsuUpdateColumnList, compareSize } from '@/utils/feature'
...
// 列字段
let columns = reactive(getColums(tableName, 'edit'))
// 表单数据
...
async function okEvent() {
  ...
  try {
    ...
    switch (true) {
      ...
      // 通用表
      default:
        httpRes = await httpApi(tableName, ruleForm.value)
        break
    }
    ...
    // 有删除操作，返回列表页面，否则刷新当前页面
    hasRemove ? moreEvent(crossData.oldTableName) : emits('fetchData')

    type != 'update' && toPayTip(httpRes)
  } catch (error) {
...
// 支付提示
function toPayTip(httpRes) {
  let { hasPay, sfsh } = tableConfigs[tableName].table
  // 待支付且不带审核时，提示支付
  if (hasPay != '' && hasPay != '否' && sfsh != '是' && isAuth(tableName, '支付')) {
    ElMessageBox.confirm('操作成功,是否前往支付?', '支付提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
      .then(() => {
        toDetailPage(tableName, httpRes.data, 1)
      })
      .catch(() => {})
  }
}
